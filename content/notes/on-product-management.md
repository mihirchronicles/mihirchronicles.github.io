...
- **Domain Modeling**: Most features fail before a single line of code is written. Not because the engineering was bad. Not because the design was ugly. Because nobody agreed on what the *things* in the system actually are. That is a domain modeling problem. Domain modeling is not database design. It is not an ERD on a whiteboard. It is the practice of understanding your customer's reality—the people, the things, and the relationships between them before you prescribe a solution. It is the step that most teams skip, and it is the reason most products feel clunky.
    - When you ask an engineer to build a feature, they make dozens of small decisions about how to store and relate data. When you have not modeled the domain first, those decisions are made in a vacuum. Engineers guess. Designers assume. And everyone builds toward a slightly different understanding of what the product is supposed to do.
    - The result is technical debt that is not really technical. It is conceptual. You end up with table structures that do not match how customers think, search that does not surface what users expect, and a codebase held together with conditional logic that compensates for a poorly understood reality.
    - The fix is not better code. It is systems thinking.
    - Stage 1: Before wireframes. Before tickets. Before anything. Sit with the customer's workflow and ask four questions:
        - Who are the people? Not user types or personas in the marketing sense. The actual roles operating in the system. An analyst. A firm. A project manager in the field. Each person has different jobs to do and different mental models of the data.
        - What are the jobs to be done? Not features. The underlying goals. “Find a company to invest in.” “Report impact data from the field.” “Compare two cleanup sites.” These jobs reveal the entry points into your system — and entry points are your entities.
        - What are the things they interact with? These are your candidate entities. Write down every noun a customer mentions in a workflow conversation. Company. Project. Impact Log. Permit. Investor. Expert Interview. Do not filter yet. Volume first.
        - What are the relationships between those things? This is where most teams stop too early. A company has many projects. A project generates many impact logs. A topic links to many companies. Map these explicitly. A line on a whiteboard is worth more than three paragraphs in a spec.
        - One practical technique: the sticky note sort. Write every noun on a yellow note. Then move attributes — things that describe an entity rather than stand alone — to blue notes and stick them inside the relevant entity. CEO name goes inside Company. GPS coordinates go inside Project, not Company, because a company operates in many locations. This is not a trivial distinction. If you attach location to the company, you have just built a system that can only ever have one location per company. That is a structural limitation that will haunt you.
    - Stage 2: Once you know your entities, you write the contract. Not a spec. Not a Figma file. A JSON draft. This is the moment you stop vibing and start defining. The schema contract translates the entities and relationships from your whiteboard into the strict rules an engineer can actually build against. It prevents the single most common source of broken products: unstructured data.
        - Use Enums, not strings, for categories. If users can type anything into a field, they will. One person types "kg." Another types "Kilos." A third types "5,000 kg." Now you cannot sum your own impact data. An Enum forces a pre-approved list. It creates a shared vocabulary at the point of entry, which is the only place it matters.
        - Split values from units. A database cannot sum the text "5 tons." You must split it into two fields: `value` as a Float and `unit` as an Enum. This is not pedantry. This is the difference between a dashboard that works and one that throws a NaN error when the CEO asks for a total.
        - Use Geopoints for location, not strings. “Los Angeles” as a string gives you a label. A Geopoint gives you a coordinate that enables map-based filtering, radius search, and spatial comparison. The data type you choose in the schema determines what the product can do. This is a product decision, not an engineering one.
        - Define constraints. Can a company exist without a name? Can an impact log have a negative value? Can a project have zero parent companies? Answer these questions in the schema. If you do not, engineers will make assumptions, and your UI will eventually surface a broken edge case in front of a customer.
        - The output of this stage is a JSON draft, not a diagram. Hand it to your engineers as a contract. It replaces the vague design spec and closes the gap between what the UI promises and what the database can deliver.
    - Stage 3: Entities and schemas get you a structured product. A knowledge graph gets you an intelligent one. A relational database stores transactions. A knowledge graph stores *relationships between concepts*. The difference matters when customers ask questions that require the system to reason across domains.
        - Consider the difference between these two answers to the question “Show me all investments fighting climate change”: 
            - *Database answer:* Returns nothing, because no company is tagged “climate change.”
            - Knowledge graph answer: Walks the connections. GreenYield practices Regenerative Farming. Regenerative Farming increases Carbon Sequestration. Carbon Sequestration fights Climate Change. GreenYield surfaces automatically, even though the phrase “climate change” appears nowhere in its profile.
        - This is called semantic interoperability. The system understands the *meaning* of the business activity, not just the text labels. Netflix built exactly this with their Unified Data Architecture—a knowledge graph that sits above their physical databases and lets users search with human language rather than exact keywords.
        - For product managers, building a knowledge graph starts with one discipline: naming your verbs.
        - The entity map gives you the nouns. The knowledge graph gives you the arrows. Write them explicitly:
            - `[Investment] → funds → [Project]`
            - `[Project] → generates → [Impact_Log]`
            - `[Company] → practices → [Topic]`
            - `[Topic] → reduces → [Outcome]`
        - These arrows are not decorative. They define what questions the system can answer automatically. If you want your product to surface insight without forcing the user to manually connect the dots, the relationships must be in the model.
        - One critical placement decision—in an innovation investment context, money should connect to the **Project**, not the **Impact Log**. If you link investment directly to a specific result, the system loses the asset that created that result. You cannot amortize the investment over time. You cannot compare cost-per-outcome across projects. The Project is the capacity. The Impact Log is the evidence. They are not the same entity, and your model must reflect that.
    - Your system design should reflect the mental model of your customer. Not the mental model of the engineer who built the first version. Not the mental model inherited from a legacy database schema. The customer's. If customers call it a Fund and your system calls it a Portfolio, you have created cognitive load. They have to translate every time they use the product. Over hundreds of interactions, that adds up to friction they cannot name but definitely feel. This is what a clunky experience actually is. It is rarely bad visual design. It is almost always a semantic gap—the distance between how the user thinks and how the system stores.
    - Domain modeling closes that gap. It is not a backend task. It is not an engineering task. It is a product management task, and it belongs at the beginning of every feature, not at the end. Build the model first. Then build the product.
...
- Strategy
    ...
    - [How Will You Measure Your Life by Clayton M. Christensen](/how-will-you-measure-your-life)
    - [What is Strategy by Harvard Business Review](https://hbr.org/1996/11/what-is-strategy)
    - [Strategy Canvas Template by Blue Ocean Strategy](https://www.blueoceanstrategy.com/blog/strategy-canvas-template/)
